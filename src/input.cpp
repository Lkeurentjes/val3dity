...
//-- ignore XML namespace
    ...
    Surface *process_gml_surface(const pugi::xml_node &n, int id, std::map<std::string, pugi::xpath_node> &dallpoly,
                                 double tol_snap, IOErrors &errs) {
    ...
    Solid *
    process_gml_solid(const pugi::xml_node &nsolid, std::map<std::string, pugi::xpath_node> &dallpoly, double tol_snap,
                      IOErrors &errs) {
        //-- exterior shell
        Solid *sol = new Solid();
    ...
    MultiSolid *process_gml_multisolid(const pugi::xml_node &nms, std::map<std::string, pugi::xpath_node> &dallpoly,
                                       double tol_snap, IOErrors &errs) {
    ...
    CompositeSolid *
    process_gml_compositesolid(const pugi::xml_node &nms, std::map<std::string, pugi::xpath_node> &dallpoly,
                               double tol_snap, IOErrors &errs) {
    ...
    MultiSurface *process_gml_multisurface(const pugi::xml_node &nms, std::map<std::string, pugi::xpath_node> &dallpoly,
                                           double tol_snap, IOErrors &errs) {
    ...
    CompositeSurface *
    process_gml_compositesurface(const pugi::xml_node &nms, std::map<std::string, pugi::xpath_node> &dallpoly,
                                 double tol_snap, IOErrors &errs) {
        ...
        for (auto &r: pgn) {
            ...
            for (auto &i: r) {
                ...
                if (j.count("transform") == 0) {
                    ...
                    x = (double(j["vertices"][i][0]) * double(j["transform"]["scale"][0])) +
                        double(j["transform"]["translate"][0]);
                    y = (double(j["vertices"][i][1]) * double(j["transform"]["scale"][1])) +
                        double(j["transform"]["translate"][1]);
                    z = (double(j["vertices"][i][2]) * double(j["transform"]["scale"][2])) +
                        double(j["transform"]["translate"][2]);
                }
                x -= _minx;
                y -= _miny;
                Point3 p3(x, y, z);
    ...
    void
    process_json_geometries_of_co(json &jco, CityObject *co, std::string coid, std::vector<GeometryTemplate *> &lsGTs,
                                  json &j, double tol_snap) {
        int idgeom = 0;
    ...
    void
    parse_cjseq(json &j, std::vector<Feature *> &lsFeatures, double tol_snap, std::vector<GeometryTemplate *> &lsGTs) {
        //-- compute (_minx, _miny)
        compute_min_xy(j);
        //-- process each CO
        for (json::iterator it = j["CityObjects"].begin(); it != j["CityObjects"].end(); ++it) {
    ...
    void process_gml_file_indoorgml(pugi::xml_document &doc, std::vector<Feature *> &lsFeatures,
                                    std::map<std::string, pugi::xpath_node> &dallpoly, IOErrors &errs,
                                    double tol_snap) {
...
//    pugi::xpath_node_set nstate = doc.select_nodes(s.c_str());
    ...
    void parse_obj(std::istream &input, std::vector<Feature *> &lsFeatures, Primitive3D prim3d, IOErrors &errs,
                   double tol_snap) {
        //-- find (minx, miny)
        std::string l;
    ...
    void
    parse_jsonfg_onefeature(json &j, std::vector<Feature *> &lsFeatures, double tol_snap, int counter, IOErrors &errs) {
    ...
    void parse_tu3djson(json &j, std::vector<Feature *> &lsFeatures, double tol_snap) {
        ...
        for (auto &f: j["features"]) {
            ...
            counter++;
            // TODO is geom needed cause always zero for tu3djson
            std::string gid = "id=" + std::to_string(counter)+ "|geom=" + std::to_string(0);
            if (f["geometry"]["type"] == "Solid") {
                Solid *s = new Solid(gid);
                bool oshell = true;
                int c = 0;
                for (auto &shell: f["geometry"]["boundaries"]) {
                    std::string shid = gid + "|" + "shell=" + std::to_string(c);
                    Surface *sh = new Surface(shid, tol_snap);
